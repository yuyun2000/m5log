...
```
audio_dir="/hy-tmp/pos"; label="<HI>"; tmp_wav=$(mktemp); tmp_txt=$(mktemp); while IFS= read -r f; do id="${f##*/}"; id="${id%.*}"; echo "$id $f" >> "$tmp_wav"; echo "$id $label" >> "$tmp_txt"; done < <(find "$audio_dir" -type f \( -iname "*.wav" -o -iname "*.flac" \) | sort); mv "$tmp_wav" wav1.scp; mv "$tmp_txt" text1
```

```
audio_dir="/hy-tmp/data/neg"
label="<FILLER>"
...
```
audio_dir="/hy-tmp/data/RIRS_NOISES"
label="<FILLER>"
...
```
join -j 1 <(cat *.scp    | sort -u) <(cat text* | sort -u) > joined && awk '{print $1" "$2 > "wav.scp"; $2=""; print > "text"}' joined
```
### 一个文件随机拆分
```
...
## 生成噪声 混响增强
```
python tools/make_lmdb.py /hy-tmp/data/noi.scp /hy-tmp/noi-lmdb
```

```
python tools/make_lmdb.py /hy-tmp/data/rir.scp /hy-tmp/rir-lmdb
```
